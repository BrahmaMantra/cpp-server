...
void TcpConnection::handle_recv() {
    ...
    if(recv_callback){
        ...
        std::cerr << "recv_callback is not set,sockfd = " <<client_sock->get_fd()<<std::endl;
...
void TcpConnection::init(Server *server) {
     std::function<void(Socket *)> close_callback = [this](Socket *_client_sock) {
        loop->delete_connection(_client_sock);
    };
    set_close_callback(close_callback);
    std:: function<void(TcpConnection *)> _recv_callback = server_handlers[server->get_handle_type()];
...
// void TcpConnection::send_heartbeat() {
//     send("hearting");
// }