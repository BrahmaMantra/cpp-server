...
// void bind(int sockfd, const struct sockaddr_in& serv_addr) {
...
// int accept(int sockfd, struct sockaddr_in& client_addr) {
...
// // 目前是阻塞且单线程的，后续优化
// void echoServer(int client_sockfd) {
//     char buf[READ_BUFFER];//由于使用非阻塞IO，读取客户端buffer，一次读取buf大小数据，直到全部读取完毕
//     while (true) {
